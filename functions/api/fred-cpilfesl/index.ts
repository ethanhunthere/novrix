/**
 * Cloudflare Pages Function — Core CPI (Excl. Food & Energy)
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_cpilfesl_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-cpilfesl] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-cpilfesl] Fallback failed:', message);
    return new Response(
