/**
 * Cloudflare Pages Function — Overnight Reverse Repurchase Agreements
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_rrpontsyd_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-rrpontsyd] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-rrpontsyd] Fallback failed:', message);
    return new Response(
