/**
 * Cloudflare Pages Function - S&P 500 Index
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM sp500_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[sp500] D1 error:', e);
  }

  try {
    const key = env.FRED_API_KEY ?? '';
    if (!key) throw new Error('FRED_API_KEY not configured');
    const params = new URLSearchParams({ series_id: 'SP500', api_key: key, file_type: 'json', observation_start: '1990-01-01' });
    ...
    console.error('[sp500] Fallback failed:', message);
    return new Response(
      JSON.stringify({ success: false, error: 'No S&P 500 data available' }),
