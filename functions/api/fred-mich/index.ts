/**
 * Cloudflare Pages Function — UMich 1-Year Inflation Expectations
 * Source: FRED series MICH
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_mich_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-mich] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-mich] Fallback failed:', message);
    return new Response(
