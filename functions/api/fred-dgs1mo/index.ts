/**
 * Cloudflare Pages Function — 1-Month Treasury Constant Maturity Rate
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_dgs1mo_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-dgs1mo] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-dgs1mo] Fallback failed:', message);
    return new Response(
