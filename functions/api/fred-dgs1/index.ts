/**
 * Cloudflare Pages Function — 1-Year Treasury Constant Maturity Rate
 * Source: FRED series DGS1
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_dgs1_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-dgs1] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-dgs1] Fallback failed:', message);
    return new Response(
