/**
 * Cloudflare Pages Function — 30-Year Fixed Rate Mortgage Average
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_mortgage30us_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-mortgage30us] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-mortgage30us] Fallback failed:', message);
    return new Response(
