/**
 * Cloudflare Pages Function — Real Gross Domestic Product
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_gdpc1_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-gdpc1] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-gdpc1] Fallback failed:', message);
    return new Response(
