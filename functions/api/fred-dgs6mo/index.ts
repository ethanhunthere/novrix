/**
 * Cloudflare Pages Function — 6-Month Treasury Constant Maturity Rate
 * Source: FRED series DGS6MO
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_dgs6mo_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-dgs6mo] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-dgs6mo] Fallback failed:', message);
    return new Response(
