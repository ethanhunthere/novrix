/**
 * Cloudflare Pages Function — Housing Starts: Total
 * Source: FRED series HOUST
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_houst_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-houst] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-houst] Fallback failed:', message);
    return new Response(
