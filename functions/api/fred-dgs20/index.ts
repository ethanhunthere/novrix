/**
 * Cloudflare Pages Function — 20-Year Treasury Constant Maturity Rate
 * Source: FRED series DGS20
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_dgs20_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-dgs20] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-dgs20] Fallback failed:', message);
    return new Response(
