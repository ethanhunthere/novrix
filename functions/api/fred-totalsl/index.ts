/**
 * Cloudflare Pages Function — Total Consumer Credit Outstanding
 * Source: FRED series TOTALSL
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_totalsl_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-totalsl] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-totalsl] Fallback failed:', message);
    return new Response(
