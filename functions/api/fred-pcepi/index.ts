/**
 * Cloudflare Pages Function — PCE Price Index
 * Source: FRED series PCEPI
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_pcepi_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-pcepi] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-pcepi] Fallback failed:', message);
    return new Response(
