/**
 * Cloudflare Pages Function — Job Openings: Total Nonfarm (JOLTS)
 * Source: FRED series JTSJOL
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_jtsjol_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-jtsjol] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-jtsjol] Fallback failed:', message);
    return new Response(
