/**
 * Cloudflare Pages Function — Core PCE Price Index
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_pcepilfe_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-pcepilfe] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-pcepilfe] Fallback failed:', message);
    return new Response(
