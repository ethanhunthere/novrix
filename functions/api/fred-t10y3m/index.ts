/**
 * Cloudflare Pages Function — 10-Year minus 3-Month Treasury Spread
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_t10y3m_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-t10y3m] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-t10y3m] Fallback failed:', message);
    return new Response(
