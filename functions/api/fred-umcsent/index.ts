/**
 * Cloudflare Pages Function — UMich Consumer Sentiment Index
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_umcsent_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-umcsent] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-umcsent] Fallback failed:', message);
    return new Response(
