/**
 * Cloudflare Pages Function — 30-Year Treasury Constant Maturity Rate
 * Source: FRED series DGS30
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_dgs30_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-dgs30] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-dgs30] Fallback failed:', message);
    return new Response(
