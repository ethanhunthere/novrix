/**
 * Cloudflare Pages Function — US Unemployment Rate
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_unrate_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-unrate] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-unrate] Fallback failed:', message);
    return new Response(
