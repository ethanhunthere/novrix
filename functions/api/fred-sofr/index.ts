/**
 * Cloudflare Pages Function — Secured Overnight Financing Rate (SOFR)
 * Source: FRED series SOFR
 * Primary: D1 database (populated by fred-cron worker).
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_sofr_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-sofr] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-sofr] Fallback failed:', message);
    return new Response(
