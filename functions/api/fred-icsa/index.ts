/**
 * Cloudflare Pages Function — Initial Jobless Claims (Weekly)
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_icsa_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-icsa] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-icsa] Fallback failed:', message);
    return new Response(
