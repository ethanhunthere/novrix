/**
 * Cloudflare Pages Function — 10-Year Breakeven Inflation Rate
 * Source: FRED series T10YIE
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_t10yie_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-t10yie] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-t10yie] Fallback failed:', message);
    return new Response(
