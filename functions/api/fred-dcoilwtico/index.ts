/**
 * Cloudflare Pages Function — WTI Crude Oil Price ($/barrel)
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_dcoilwtico_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-dcoilwtico] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-dcoilwtico] Fallback failed:', message);
    return new Response(
