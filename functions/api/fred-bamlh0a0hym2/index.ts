/**
 * Cloudflare Pages Function — ICE BofA High Yield Option-Adj Spread
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_bamlh0a0hym2_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-bamlh0a0hym2] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-bamlh0a0hym2] Fallback failed:', message);
    return new Response(
