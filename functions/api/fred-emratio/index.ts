/**
 * Cloudflare Pages Function — Employment-Population Ratio
 * Source: FRED series EMRATIO
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_emratio_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-emratio] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-emratio] Fallback failed:', message);
    return new Response(
