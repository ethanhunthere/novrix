/**
 * Cloudflare Pages Function — Total Nonfarm Payrolls
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_payems_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-payems] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-payems] Fallback failed:', message);
    return new Response(
