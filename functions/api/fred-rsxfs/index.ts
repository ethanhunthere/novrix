/**
 * Cloudflare Pages Function — Advance Retail Sales (Excl. Food Svc)
 * Source: FRED series RSXFS
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_rsxfs_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-rsxfs] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-rsxfs] Fallback failed:', message);
    return new Response(
