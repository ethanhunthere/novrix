/**
 * Cloudflare Pages Function — 5-Year Breakeven Inflation Rate
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_t5yie_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-t5yie] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-t5yie] Fallback failed:', message);
    return new Response(
