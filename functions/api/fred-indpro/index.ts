/**
 * Cloudflare Pages Function — Industrial Production Index
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_indpro_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-indpro] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-indpro] Fallback failed:', message);
    return new Response(
