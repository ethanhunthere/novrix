/**
 * Cloudflare Pages Function — Fed Balance Sheet (Total Assets)
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_walcl_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-walcl] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-walcl] Fallback failed:', message);
    return new Response(
