/**
 * Cloudflare Pages Function — OECD M3 Proxy (US Broad Money)
 * Source: FRED series MABMM301USM189S
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT date, value FROM fred_mabmm301_data ORDER BY date ASC`
    ).all<DataRecord>();
    ...
    console.error('[fred-mabmm301usm189s] D1 error:', e);
  }
  ...
  try {
    ...
    console.error('[fred-mabmm301usm189s] Fallback failed:', message);
    return new Response(
