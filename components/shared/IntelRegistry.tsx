...
interface IntelRegistryProps {
  ...
  /** Primary theme accent used for header underline, focus states, caret color. Defaults to sentiment blue (#3B82F6). */
  themeAccent?: string;
  /** Optional callback invoked when an indicator item is clicked. Receives the item id. */
  onItemClick?: (id: string) => void;
}

/**
 * Shared Intel Registry sidebar — matches the sentiment page Intel Registry
 * design, layout, fonts, panel structure, and behavior exactly.
 * Accepts a themeAccent override so consuming pages (e.g. metrilytics) can
 * apply their own palette while keeping everything else identical.
 */
const IntelRegistry = memo(function IntelRegistry({
  registryCats,
  registrySearch,
  setRegistrySearch,
  openAccordion,
        ...
        <div className="px-5 pt-5 pb-4" style={{ background: '#080A0F', borderBottom: '1px solid #1A2030' }}>
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex flex-col gap-1.5">
              ...
              <div style={{ width: '34px', height: '2px', background: themeAccent, borderRadius: '1px' }} />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              style={{
                ...
                outline: 'none',
                caretColor: themeAccent,
                boxSizing: 'border-box',
                transition: 'border-color 180ms ease, box-shadow 180ms ease',
              }}
              onFocus={(e) => {
                e.currentTarget.style.borderColor = themeAccent + '88';
                e.currentTarget.style.boxShadow = `0 0 0 3px ${themeAccent}14`;
              }}
              onBlur={(e) => {
                  ...
                  {(q ? visibleIndicators : cat.items).map((item, idx) => {
                    ...
                    return (
                      <div
                        ...
                        role="button"
                        onClick={() => {
                          setSelectedIndicator(indItem.id);
                          if (onItemClick) {
                            onItemClick(indItem.id);
                          } else {
...
export type { IndicatorEntry, DividerEntry, AnyEntry };
